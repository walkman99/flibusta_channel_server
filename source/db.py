...
class FlibustaChannelDB:
    ...
    async def prepare(cls, app):
        cls.pool = await asyncpg.create_pool(user=Config.DB_USER, password=Config.DB_PASSWORD,
                                             host=Config.DB_HOST, database=Config.DB_DATABASE)

    @classmethod
    async def set_message_id(cls, book_id: int, file_type: str, message_id: int):
