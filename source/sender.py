...
class Book:
    ...
    async def get_by_id(book_id: int) -> "Book":
        async with request("GET", f"http://{Config.FLIBUSTA_SERVER_HOST}:{Config.FLIBUSTA_SERVER_PORT}/book/{book_id}") as response:
...
class Sender:
    ...
    async def prepare(self):
        ...
        self.flibusta_server_pool = await asyncpg.create_pool(user=Config.FLIBUSTA_SERVER_USER, password=Config.FLIBUSTA_SERVER_PASSWORD,
                                                              database=Config.FLIBUSTA_SERVER_DATABASE, host=Config.FLIBUSTA_SERVER_HOST)


    async def upload(self, book_id: int, file_type: str):
        print(f"Download {book_id} {file_type}...")
        try:
            async with request("GET", f"http://{Config.FLIBUSTA_SERVER_HOST}:{Config.FLIBUSTA_SERVER_PORT}/book/download/{book_id}/{file_type}") as response:
